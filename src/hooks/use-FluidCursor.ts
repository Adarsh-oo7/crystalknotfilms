...
const useFluidCursor = () => {
  ...
  function getWebGLContext(canvas: HTMLCanvasElement) {
    ...
    // Type assertion to ensure TypeScript knows this is a WebGL context
  ...
  function clickSplat(pointer: Pointer) {
    ...
    const rgbColor = { r: color[0] * 10.0, g: color[1] * 10.0, b: color[2] * 10.0 };
    let dx = 10 * (Math.random() - 0.5);
    let dy = 30 * (Math.random() - 0.5);
    splat(pointer.texcoordX, pointer.texcoordY, dx, dy, rgbColor);
  }
  ...
  function updatePointerMoveData(pointer: Pointer, posX: number, posY: number, color: [number, number, number] | { r: number; g: number; b: number }) {
    pointer.prevTexcoordX = pointer.texcoordX;
    pointer.prevTexcoordY = pointer.texcoordY;
    pointer.texcoordX = posX / canvas.width;
    pointer.texcoordY = 1.0 - posY / canvas.height;
