...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
  return (
    <motion.div
      ...
      className={cn(
        "relative z-[60] mx-auto hidden w-full max-w-7xl flex-row items-center justify-between self-start rounded-full  px-4 py-2 lg:flex bg-black ",
...
export const NavItems = ({ items, className, onItemClick }: NavItemsProps) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "absolute inset-0 hidden flex-1 flex-row items-center justify-center space-x-2 text-sm text-white transition duration-200 lg:flex lg:space-x-2",
        className
      )}
  ...
  return (
    ...
    >
      <span className="font-medium text-white dark:text-white">
        ...
        <span style={{ fontFamily: "Essential Sans, sans-serif" }}>Films</span>
      </span>
    </Link>
  );
};
